...
Unity 기반 공정 자동화 시뮬레이션으로,  
**enum 기반 상태 제어(State-driven Architecture)** 와 **Fault 전파 로직(Fault Propagation)** 을 직접 설계했습니다.  
**Unity Client – ASP.NET Core Web API – SQLite DB** 구조로 공정 상태를 서버와 동기화하며, JSON으로 데이터 로그를 로컬에 별도 기록합니다.
...
- **KPI 실시간 표시** — Total / OK / NG / Fault Count를 Unity UI에 시각화
